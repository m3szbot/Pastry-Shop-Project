...
#define HTC_MAG_ING_STARTING_SIZE 30373
#define HTC_MAG_RIC_STARTING_SIZE 95373
...
{
    // TODO: REMOVE nome_ingrediente, use lotti->nome_ingrediente
    // key = nome_ingrediente
    char nome_ingrediente[LEN];
...
minheap_lotti_t *create_minheap_lotti(int size, char *ingrediente)
{
    minheap_lotti_t *h = malloc(sizeof(minheap_lotti_t));
    if (h != NULL)
    {
        strcpy(h->nome_ingrediente, ingrediente);
        h->current_size = 0;
        h->max_size = size;
        // allocate array with defined size
...
int search_ht_ing_ric(ht_ing_ric_t *h, char *s_key)
{
    ...
    while (i < h->max_size)
    {
        ...
        if (h->slots[try_index] == NULL)
            return -1;
        // compare keys
        if (strcmp(s_key, h->slots[try_index]->nome_ingrediente) == 0)
            return try_index;
        i++;
    }
    return -1;
...
void insert_ht_ing_ric(ht_ing_ric_t *h, char *ingrediente, int quantita)
{
    if (h->current_size == h->max_size)
    {
    ...
    else
    {
        ...
        while (i < h->max_size)
        {
            try = hash_ht_ing_ric(tmp->nome_ingrediente, h->max_size, i);
...
// return node of searched key, or NULL if not found
htc_mag_ing_item_t *search_htc_mag_ing(htc_mag_ing_t *h, char *s_key)
{
    int index;
    index = hash_htc_mag_ing(s_key, h->max_size);
    ...
    else
    {
        while (current != NULL)
        {
            if (strcmp(current->nome_ingrediente, s_key) == 0)
                return current;
            else
                current = current->next;
        }
...
void insert_htc_mag_ing(htc_mag_ing_t *h, char *new_key, minheap_lotti_t *new_value)
{
    h->load_factor = 1.0 * (h->current_size + 1) / h->max_size;
    if (h->load_factor > LOAD_FACTOR)
        rehash_htc_mag_ing(h);
    // allocate new item
    int index = hash_htc_mag_ing(new_key, h->max_size);
    ...
    else
    {
        ...
        strcpy(tmp->nome_ingrediente, new_key);
        tmp->lotti_ingrediente = new_value;
...
// delete ingredient from mag_ingredienti
void delete_htc_mag_ing(htc_mag_ing_t *h, char *key)
{
    int index;
    index = hash_htc_mag_ing(key, h->max_size);
    htc_mag_ing_item_t *prev = NULL, *current = NULL;
    // start from slot head until slot linked list end
    current = h->slots[index];
    while (current != NULL)
    {
        // if node with matching key found, delete node
        if (strcmp(current->nome_ingrediente, key) == 0)
        {
            // if head node
            if (prev == NULL)
            ...
            // free current node (and its pointed data)
            free(current);
            h->current_size--;
            h->load_factor = 1.0 * (h->current_size) / h->max_size;
            return;
...
void walk_minheap_lotti(minheap_lotti_t *h)
{
    ...
    for (i = 0; i < h->current_size; i++)
    {
        printf("%s ", h->arr[i]->nome_ingrediente);
...
void controlla_scadenza_ingrediente(int tempo, htc_mag_ing_t *mag_ingredienti, minheap_lotti_t *lotti_current_ing)
{
    ...
    // if no lots for ingredient, remove from mag_ingredienti
    if (peeked == NULL)
        delete_htc_mag_ing(mag_ingredienti, lotti_current_ing->nome_ingrediente);
}
...
void flush_tutti_lotti_scaduti(int tempo, int periodo, htc_mag_ing_t *mag_ingredienti)
{
    ...
    if (tempo % periodo == 0)
    {
        for (i = 0; i < mag_ingredienti->max_size; i++)
        {
            ...
            {
                controlla_scadenza_ingrediente(tempo, mag_ingredienti, mag_ingredienti->slots[i]->lotti_ingrediente);
            }
        }
    }
}
...
int controlla_quantita_ingrediente(int tempo, htc_mag_ing_t *mag_ingredienti, minheap_lotti_t *lotti_current_ing, int quantita_richiesta)
{
    int i, sum = 0;
    if (lotti_current_ing->current_size > 0)
    {
        // delete lotti scaduti
        controlla_scadenza_ingrediente(tempo, mag_ingredienti, lotti_current_ing);
...
int controlla_ingredienti_ordine(int tempo, htc_mag_ric_t *mag_ricette, htc_mag_ing_t *mag_ingredienti, ordine_t *ordine)
{
    ...
    char current_ing[LEN];
    // cycle through ingredients (hash table oa, slots always full) of ricetta
    // PROBLEM HERE: nodo_ricetta=NULL!!!
    for (i = 0; i < ordine->p_ricetta->ingredienti_ricetta->max_size; i++)
    {
        // find current_ingredient
        ingrediente_ricetta_t *nodo_ingrediente = ordine->p_ricetta->ingredienti_ricetta->slots[i];
        strcpy(current_ing, nodo_ingrediente->nome_ingrediente);
        quantita_richiesta = nodo_ingrediente->quantita_ingrediente * ordine->quantita_ordine;
        // find minheap_lotti of current ingredient in mag_ingredienti
        htc_mag_ing_item_t *ingredient_item = search_htc_mag_ing(mag_ingredienti, current_ing);
        // if ingredient not found in magazzino ingredienti
        if (ingredient_item == NULL)
            return -1;
        minheap_lotti_t *lotti_current_ing = ingredient_item->lotti_ingrediente;
        // check availability of current ingredient, return -1 if current not available
        if (controlla_quantita_ingrediente(tempo, mag_ingredienti, lotti_current_ing, quantita_richiesta) == -1)
            return -1;
    }
...
void consuma_ingredienti_ordine(htc_mag_ric_t *mag_ricette, htc_mag_ing_t *mag_ingredienti, ordine_t *ordine)
{
    ...
    for (i = 0; i < ordine->p_ricetta->ingredienti_ricetta->max_size; i++)
    {
        ...
        strcpy(current_ing, nodo_ingrediente->nome_ingrediente);
        quantita_richiesta = nodo_ingrediente->quantita_ingrediente * ordine->quantita_ordine;
        minheap_lotti_t *lotti_current_ing = search_htc_mag_ing(mag_ingredienti, current_ing)->lotti_ingrediente;
...
void aggiungi_ricetta(ll_input_ar_t *list, htc_mag_ric_t *mag_ricette)
{
    ...
    if (scanf("%s%*c", ricetta) > 0)
    {
        ...
        {
            // put ingredients into hash table ht_ing_ric as ingrediente_ricetta
            returned = pop_input_ar(list);
            peso_ricetta += returned->quantita;
            insert_ht_ing_ric(ht_ingredienti, returned->ingrediente, returned->quantita);
            free(returned);
        }
    ...
    while (whitespace != '\n')
    {
        ...
        if (scanf("%s %d %d%c", ingrediente, &quantita, &scadenza, &whitespace) > 0)
        {
            ...
            if (searched == NULL)
            {
                ...
                minheap_lotti_t *new_minheap = create_minheap_lotti(MINHEAP_LOTTI_SIZE, ingrediente);
...
int main()
{
    ...
    while (!flag_eof)
    {
        ...
        // always flush whitespace when reading with %*c - remains in stdin
        {
            ...
            {
                aggiungi_ricetta(list_input_ar, mag_ricette);
