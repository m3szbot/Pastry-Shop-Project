...
#define MINHEAP_ORDINI_SIZE 64
#define MINHEAP_LOTTI_SIZE 8
#define HTC_MAG_ING_STARTING_SIZE 33133
#define HTC_MAG_RIC_STARTING_SIZE 33133
...
typedef struct lotto_s
{
    ...
    int quantita_ingrediente_lotto;
    int scadenza;
} lotto_t;
...
typedef struct htc_mag_ing_item_s
{
    ...
    int quantita_totale_ingrediente;
...
typedef struct ingrediente_ricetta_s
{
    ...
    // value=quantita
    int quantita_ingrediente_ricetta;
...
typedef struct ht_ing_ric_s
{
    ...
    ingrediente_ricetta_t *bottleneck_ingrediente;
    int current_size;
    int max_size;
...
lotto_t *create_node_lotto(int quantita, int scadenza)

{
    lotto_t *tmp = malloc(sizeof(lotto_t));
    if (tmp != NULL)
    {
        tmp->quantita_ingrediente_lotto = quantita;
...
void insert_ht_ing_ric(htc_mag_ing_t *mag_ingredienti, ht_ing_ric_t *h, htc_mag_ing_item_t *p_ingrediente, int quantita)
{
    ...
    else
    {
        ...
        tmp->quantita_ingrediente_ricetta = quantita;
...
void insert_htc_mag_ing(htc_mag_ing_t *h, char *new_ing, minheap_lotti_t *minheap_lotti)
{
    ...
    else
    {
        ...
        tmp->quantita_totale_ingrediente = 0;
...
void walk_minheap_lotti(minheap_lotti_t *h)
{
    ...
    for (i = 0; i < h->current_size; i++)
    {
        printf("scadenza: %d, quantity: %d\n", h->arr[i]->scadenza, h->arr[i]->quantita_ingrediente_lotto);
    }
    printf("\n");
}
...
void controlla_scadenza_ingrediente(int tempo, htc_mag_ing_item_t *node_ingrediente)
{
    lotto_t *peeked = peek_minheap_lotti(node_ingrediente->lotti_ingrediente);
    while (peeked != NULL && tempo >= peeked->scadenza)
    {
        // decrease quantita_totale di ingrediente
        node_ingrediente->quantita_totale_ingrediente -= peeked->quantita_ingrediente_lotto;
        pop_minheap_lotti(node_ingrediente->lotti_ingrediente);
        peeked = peek_minheap_lotti(node_ingrediente->lotti_ingrediente);
    }
}
    ...
    if (tempo % periodo == 0)
    {
        for (i = 0; i < mag_ingredienti->max_size; i++)
        {
            if (mag_ingredienti->slots[i] != NULL)
                controlla_scadenza_ingrediente(tempo, mag_ingredienti->slots[i]);
        }
    }
}

// return 1 if enough quantity of current ing, -1 otherwise
int controlla_quantita_ingrediente(int tempo, htc_mag_ing_item_t *node_ingrediente, int quantita_richiesta)
{
    if (node_ingrediente->lotti_ingrediente->current_size > 0)
    {
        // delete lotti scaduti
        controlla_scadenza_ingrediente(tempo, node_ingrediente);
        // check if enough quantita of ingredient
        if (node_ingrediente->quantita_totale_ingrediente >= quantita_richiesta)
            return 1;
    }
    return -1;
}
...
int controlla_ingredienti_ordine(int tempo, htc_mag_ric_t *mag_ricette, htc_mag_ing_t *mag_ingredienti, ordine_t *ordine)
{
    ...
    // check bottleneck_ingrediente
    ingrediente_ricetta_t *bottleneck_ingrediente = ordine->p_ricetta->ingredienti_ricetta->bottleneck_ingrediente;
    if (bottleneck_ingrediente != NULL)
    {
        quantita_richiesta = bottleneck_ingrediente->quantita_ingrediente_ricetta * ordine->quantita_ordine;
        if (controlla_quantita_ingrediente(tempo, bottleneck_ingrediente->p_ingrediente, quantita_richiesta) == -1)
            return -1;
    }
    ...
    for (i = 0; i < ordine->p_ricetta->ingredienti_ricetta->max_size; i++)
    {
        ...
        // if minheap of ingredient empty
        if (current_ing_ricetta->p_ingrediente->lotti_ingrediente->current_size == 0)
            return -1;
        quantita_richiesta = current_ing_ricetta->quantita_ingrediente_ricetta * ordine->quantita_ordine;
        //  check availability of current ingredient, return -1 if current not available
        if (controlla_quantita_ingrediente(tempo, current_ing_ricetta->p_ingrediente, quantita_richiesta) == -1)
        {
            // set bottleneck_ingrediente
            ordine->p_ricetta->ingredienti_ricetta->bottleneck_ingrediente = current_ing_ricetta;
            return -1;
        }
    }
...
void consuma_ingredienti_ordine(htc_mag_ric_t *mag_ricette, htc_mag_ing_t *mag_ingredienti, ordine_t *ordine)
{
    ...
    for (i = 0; i < ordine->p_ricetta->ingredienti_ricetta->max_size; i++)
    {
        ...
        while (quantita_richiesta > 0)
        {
            ...
            quantita_lotto = first_lot->quantita_ingrediente_lotto;
            // if quantita_richiesta satisfied in first lost
            if (quantita_richiesta < quantita_lotto)
            {
                first_lot->quantita_ingrediente_lotto -= quantita_richiesta;
                current_ing_ricetta->p_ingrediente->quantita_totale_ingrediente -= quantita_richiesta;
                quantita_richiesta = 0;
            }
            // if quantita_richiesta >= quantita_lotto, consume whole lot
            else
            {
                current_ing_ricetta->p_ingrediente->quantita_totale_ingrediente -= quantita_lotto;
...
void rifornimento_add_new_lots(int tempo, htc_mag_ric_t *mag_ricette, htc_mag_ing_t *mag_ingredienti)
{
    ...
    while (whitespace != '\n')
    {
        ...
        if (scanf("%s %d %d%c", ingrediente, &quantita, &scadenza, &whitespace) > 0)
        {
            ...
            lotto_t *new_lotto = create_node_lotto(quantita, scadenza);
            insert_minheap_lotti(searched->lotti_ingrediente, new_lotto);
            searched->quantita_totale_ingrediente += quantita;
        }
    }
}
